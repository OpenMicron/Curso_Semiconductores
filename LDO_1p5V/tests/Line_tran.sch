...
C {vsource.sym} 60 -310 0 0 {name=VIN value="PULSE(1.62 1.98 5u 1u 1u 20u 40u)" savecurrent=false}
...
C {code.sym} 150 -630 0 0 {name=LINE_TRAN only_toplevel=false value="
...
echo ==========================================
echo ===== LDO LINE TRANSIENT ANALYSIS =====
echo ==========================================
...
* Fuente VIN recomendada en el esquemático:
* VIN PULSE(1.62 1.98 5u 1u 1u 20u 40u)
*
* Carga fija recomendada:
* ILOAD1 DC 1m

tran 10n 100u

echo ==========================================
echo ===== LINE TRANSIENT MEASUREMENTS =====
echo ==========================================
...
****************************************************
* STEP 1: VIN RISE 1.62V -> 1.98V
* Starts at 5us, ends at 6us
****************************************************

meas tran vout_max_step1 max v(VOUT) from=5u to=26u
meas tran vout_min_step1 min v(VOUT) from=5u to=26u

let overshoot_step1_mv  = (vout_max_step1 - 1.5)*1000
let undershoot_step1_mv = (1.5 - vout_min_step1)*1000

* Settling after VIN rise:
* If VOUT goes above nominal, measure when it returns below upper band.
meas tran t_settle_step1_abs when v(VOUT)=1.515 fall=1 from=5u to=26u

let t_settle_step1 = t_settle_step1_abs - 5u
let t_settle_step1_us = t_settle_step1*1e6

****************************************************
* STEP 2: VIN FALL 1.98V -> 1.62V
* Starts at 26us, ends at 27us
****************************************************

meas tran vout_max_step2 max v(VOUT) from=26u to=45u
meas tran vout_min_step2 min v(VOUT) from=26u to=45u

let overshoot_step2_mv  = (vout_max_step2 - 1.5)*1000
let undershoot_step2_mv = (1.5 - vout_min_step2)*1000

* Settling after VIN fall:
* If VOUT goes below nominal, measure when it returns above lower band.
meas tran t_settle_step2_abs when v(VOUT)=1.485 rise=1 from=26u to=45u

let t_settle_step2 = t_settle_step2_abs - 26u
let t_settle_step2_us = t_settle_step2*1e6

****************************************************
* STEP 3: SECOND VIN RISE 1.62V -> 1.98V
* Starts at 45us, ends at 46us
****************************************************

meas tran vout_max_step3 max v(VOUT) from=45u to=66u
meas tran vout_min_step3 min v(VOUT) from=45u to=66u

let overshoot_step3_mv  = (vout_max_step3 - 1.5)*1000
let undershoot_step3_mv = (1.5 - vout_min_step3)*1000

meas tran t_settle_step3_abs when v(VOUT)=1.515 fall=1 from=45u to=66u

let t_settle_step3 = t_settle_step3_abs - 45u
let t_settle_step3_us = t_settle_step3*1e6

****************************************************
* STEP 4: SECOND VIN FALL 1.98V -> 1.62V
* Starts at 66us, ends at 67us
****************************************************

meas tran vout_max_step4 max v(VOUT) from=66u to=85u
meas tran vout_min_step4 min v(VOUT) from=66u to=85u

let overshoot_step4_mv  = (vout_max_step4 - 1.5)*1000
let undershoot_step4_mv = (1.5 - vout_min_step4)*1000

meas tran t_settle_step4_abs when v(VOUT)=1.485 rise=1 from=66u to=85u
...
echo ===== STEP 1: VIN 1.62V -> 1.98V =====
print vout_max_step1
print vout_min_step1
print overshoot_step1_mv
print undershoot_step1_mv
print t_settle_step1_us

echo ===== STEP 2: VIN 1.98V -> 1.62V =====
print vout_max_step2
print vout_min_step2
print overshoot_step2_mv
print undershoot_step2_mv
print t_settle_step2_us

echo ===== STEP 3: VIN 1.62V -> 1.98V =====
print vout_max_step3
print vout_min_step3
print overshoot_step3_mv
print undershoot_step3_mv
print t_settle_step3_us

echo ===== STEP 4: VIN 1.98V -> 1.62V =====
print vout_max_step4
print vout_min_step4
print overshoot_step4_mv
print undershoot_step4_mv
